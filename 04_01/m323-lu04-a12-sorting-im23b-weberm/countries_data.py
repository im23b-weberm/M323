...
def sort_countries_by_name(data1):
    """Return a copy of the data, sorted by name."""
    # Sort countries by their names
    return sorted(data1, key=lambda x: x["name"])


def sort_countries_by_capital(data2):
    """Return a copy of the data, sorted by capital."""
    # Sort countries by their capitals
    return sorted(data2, key=lambda x: x["capital"])


def sort_countries_by_population(data3):
    """Return a copy of the data, sorted by population."""
    # Sort countries by their population
    return sorted(data3, key=lambda x: x["population"], reverse=True)


def get_ten_most_spoken_languages(data4):
    """
    ...
    """
    all_languages = (lang for country in data4 for lang in country["languages"])
    return Counter(all_languages).most_common(10)


def get_ten_most_populated_countries(data5):
    """
    Return the ten most populated countries.
    As Array with tuples like (name, population)
    Example: [('China', 1377422166), ('India', 1295210000), ...]
    """
    countries = [(country["name"], country["population"]) for country in data5]
    return sorted(countries, key=lambda x: x[1], reverse=True)[:10]
