...
def test_update_price():
    ...
    updated_list = main.update_prices(main.products, new_prices)
    for item in updated_list:
        assert item['price'] == new_prices.get(
...
def test_original_list_unchanged():
    ...
    assert (
        main.products == original_list
    ), f'Expected the original list to remain unchanged, but got {products}.'
...
def test_calculate_total():
    ...
    expected_cost = sum(
        item['quantity'] * new_prices.get(item['product'], item['price'])
        for item in updated_list
    )
    assert (
        total_cost == expected_cost
    ), f'Expected {expected_cost}, but got {total_cost}.'
